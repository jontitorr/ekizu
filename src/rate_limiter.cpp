...
Result<net::HttpResponse> RateLimiter::send(const DiscordApiRequest &req) {
	...
	for (uint8_t i{}; i < max_retries; ++i) {
		...
		if (res->headers.find("X-RateLimit-Limit") != res->headers.end() &&
			res->headers.find("X-RateLimit-Remaining") != res->headers.end() &&
			res->headers.find("X-RateLimit-Reset") != res->headers.end()) {
			rate_limits.insert_or_assign(
				req.inner.path,
				RateLimit{
					static_cast<uint16_t>(
						std::stoul(res->headers.at("X-RateLimit-Limit"))),
					static_cast<uint16_t>(
						std::stoul(res->headers.at("X-RateLimit-Remaining"))),
					std::chrono::system_clock::from_time_t(
						std::stol(res->headers.at("X-RateLimit-Reset"))),
				});
		}
	...
	return tl::make_unexpected(std::make_error_code(std::errc::protocol_error));
}
}  // namespace ekizu
