...
void Shard::log(std::string_view msg, LogLevel level) {
	...
	const auto debug_event = event_from_str(nlohmann::json{
		{"t", "LOG"}, {"d", {{"message", msg}, {"level", level}}}});
...
Result<void> Shard::connect(const std::function<void(Event)> &cb) {
	...
	if (m_websocket) {
		if (const auto res = disconnect(Shard::DISCORD_CLOSE_CODE); !res) {
			return res;
		}
	}
	...
	builder.with_auto_reconnect(true)
		.with_on_close(
			[this](net::WebSocketCloseCode code, std::string_view reason) {
				m_inflater.reset();
		...
		.with_on_connect([this, compression_enabled] {
			if (compression_enabled) {
				...
				m_inflater = std::make_unique<Inflater>(std::move(*inflater));
			}
		...
		builder.with_on_message([this](net::WebSocketMessage msg) {
			const auto is_msg_zlib_compressed =
				[](tcb::span<const std::byte> m) {
					...
					const auto expected = tcb::span<const std::byte, 4>{
						{std::byte{0x00}, std::byte{0x00}, std::byte{0xFF},
						 std::byte{0xFF}}};

					return std::equal(
						last.begin(), last.end(), expected.begin());
				};
	...
	if (const auto res = m_websocket->run(); !res) { return res; }
