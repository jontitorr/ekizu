...
template <> struct DeleterOf<z_stream> {
...
struct Inflater::Impl {
	explicit Impl(UniqueZStream stream_)
...
Inflater::Inflater(std::unique_ptr<Impl> impl)
...
Inflater::inflate(tcb::span<const std::byte> data)
{
	...
	do {
		m_impl->stream->avail_out =
			static_cast<uint32_t>(m_buffer.size());
		m_impl->stream->next_out =
			reinterpret_cast<uint8_t *>(m_buffer.data());

		if (const auto res =
			    ::inflate(m_impl->stream.get(), Z_NO_FLUSH);
		    res != Z_OK) {
		...
		result.insert(result.end(), m_buffer.begin(),
			      m_buffer.begin() + have);
