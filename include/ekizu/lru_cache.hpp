...
struct LruCache {
	...
	void put(Key &&key, Value &&value)
	{
		...
		m_cache_list.emplace_front(std::piecewise_construct,
     ...
     */
	template <typename Callback> void get(const K &key, Callback &&cb)
	{
		...
		m_cache_list.splice(m_cache_list.begin(), m_cache_list,
				    it->second);
		cb(it->second->second);
	}
    ...
    private:
	template <typename Key> void update_lru(Key &&key)
	{
