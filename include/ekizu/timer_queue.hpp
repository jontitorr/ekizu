...
struct TimerQueue {
	...
	size_t add_task(const std::function<void()> &task,
		...
		std::unique_lock lk{m_mtx};
		const auto id =
			++m_task_counter == 0 ? ++m_task_counter : m_task_counter;

		m_tasks.push(
			{id, delay, std::chrono::steady_clock::now() + delay, task});
		lk.unlock();
		m_cv.notify_one();
	...
	void remove_task(size_t id) {
		...
		if (task_it != tasks.end()) { tasks.erase(task_it); }
	}
   ...
   private:
	...
	void run() {
		...
		while (!m_stop) {
			if (m_tasks.empty()) {
				m_cv.wait(lk, [this] { return !m_tasks.empty() || m_stop; });
			} else {
				if (const auto time = m_tasks.top().time;
					...
					if (task.id != 0) { task.func(); }

					lk.lock();

					if (m_repeat_mode && task.id != 0) {
						task.time =
							std::chrono::steady_clock::now() + task.delay;
	...
	struct Queue
		...
		std::vector<Task> &get_container() { return c; }
